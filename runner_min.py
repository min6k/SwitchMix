...
class SemiRunner(DefaultRunner):
    ...
    def run_step(self):
        ...
        # ### Step 2 . 1 train IR teacher
            ...
            mix_ratio = 0.2 - (self.iter - self.step2_start) / 30000 * 0.2
            mix_generator = BlockMaskGenerator(mask_ratio=mix_ratio, mask_block_size=64)
            for ss, us in zip(sup_strong, unsup_strong):
                us['image'] = mix_generator.mask_image_mix_resize(us['image'].unsqueeze(0),
                                                                   ss['image'].unsqueeze(0)).squeeze(0)
            if (self.iter > self.iter_min) and (self.iter <= self.iter_min + self.rgb_ir_change_ir ):
                ...
                #swap_weight(self.ema_model.model, self.model)
                #print('model SWAP!')
            ...
            if (self.iter > self.iter_min + self.rgb_ir_change_ir) and (self.iter <= self.iter_min + self.rgb_ir_change_total):
                ...
                mask_ratio = 0.3 - (self.iter - self.step2_start) / 30000 * 0.3
                mask_generator = BlockMaskGenerator(mask_ratio=mask_ratio, mask_block_size=64)
                for i in sup_strong:
                    i['image'] = mask_generator.mask_image(i['image'].unsqueeze(0)).squeeze(0)
                #mask_sup_strong = mask_generator.mask_image(sup_strong)
                ...
                #swap_weight(self.ema_model_ir.model, self.model)
                #print('model_ir SWAP!')
...
class BlockMaskGenerator:
    ...
    def mask_image_mix(self, imgs, tgt_imgs):
        input_mask = self.generate_mask(imgs)
        # print(" mask size: ", input_mask.shape)
        # print(" mask :", input_mask)
        return imgs * input_mask + tgt_imgs * (1 - input_mask)

    def mask_image_mix_resize(self, imgs, tgt_imgs):
        input_mask = self.generate_mask(imgs)
        # print(" mask size: ", input_mask.shape)
        # print(" mask :", input_mask)
        #### ratio 0.1 ==> 0.9 images, 0.1 tgt_images
        return imgs * input_mask + resize(copy.deepcopy(tgt_imgs), size=imgs.shape[2:4]) * (1 - input_mask)

    def mask_image_mix_2imgs(self, imgs, tgt_imgs, imgs_str, tgt_imgs_str):
        input_mask = self.generate_mask(imgs)
        # print(" mask size: ", input_mask.shape)
        # print(" mask :", input_mask)
        #### ratio 0.9 ==> 0.1 images, 0.9 tgt_images
        return imgs * input_mask + tgt_imgs * (1 - input_mask), imgs_str * input_mask + tgt_imgs_str * (1 - input_mask), input_mask

    def mask_image_mix_2imgs_only(self, imgs, tgt_imgs, imgs_str, tgt_imgs_str):
        input_mask = self.generate_mask(imgs)
        # print(" mask size: ", input_mask.shape)
        # print(" mask :", input_mask)
        ##### ratio 0.9: 0.1 * image + 0.9 * tgt_image
        return imgs * input_mask + tgt_imgs * (1 - input_mask), imgs_str * input_mask + tgt_imgs_str * (1 - input_mask)

    def mask_image_with_mask(self, imgs, input_mask=None):
...
def swap_weight(modelS, modelT, with_guassin_noise=False):
    modelS_dict = modelS.state_dict()
    modelT_dict = modelT.state_dict()
    for key, value in list(modelS_dict.items()):
        if key in modelT_dict:
            modelS_dict[key].copy_(modelT_dict[key])
